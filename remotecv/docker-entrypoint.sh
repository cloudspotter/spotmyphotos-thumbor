...
if [ "$1" = 'remotecv' ]; then
    exec python -m remotecv.worker --host $REMOTECV_REDIS_HOST --port $REMOTECV_REDIS_PORT --database $REMOTECV_REDIS_DATABASE -l $REMOTECV_LOG_LEVEL -o $REMOTECV_LOADER -s $REMOTECV_STORE $TIMEOUT_PARAMETER $SENTRY_URL_PARAMETER
fi
